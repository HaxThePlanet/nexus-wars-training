...
def main():
  ...
  if FLAGS.algorithm == "deepq":
    ...
    with sc2_env.SC2Env(
        map_name="Simple64",
        players = [sc2_env.Agent(race=sc2_env.Race.random,name="player1"), sc2_env.Bot([sc2_env.Race.zerg, sc2_env.Race.protoss, sc2_env.Race.terran],
...
def deepq_callback(locals, globals):
  ...
  if 'done' in locals and locals['done'] == True:
    if 'mean_100ep_reward' in locals and locals['num_episodes'] >= 10\
      ...
      act_x = deepq_nexus_wars.ActWrapper(locals['act_x'])
      act_y = deepq_nexus_wars.ActWrapper(locals['act_y'])
